...
def classify(ex):
    ...
    return {
        ...
        "difficulty": float(output.get("difficulty", 0)) if output.get("difficulty") is not None else None,
        "importance": float(output.get("importance", 0)) if output.get("importance") is not None else None,
        "duration": float(output.get("duration", 0)) if output.get("duration") is not None else None,
...
def plot_overview(records, out_dir, cfg):
    ...
    fig.suptitle(f"VM.AI Dataset Overview — {cfg['title_suffix']}", color="#f0f6fc", fontsize=16, fontweight="bold", y=0.95)
    ax1 = fig.add_subplot(gs[0, 0])
    labels, sizes = zip(*cat_counts.most_common())
    colors = plt.cm.tab10(range(len(labels)))
    ax1.pie(sizes, labels=labels, autopct="%1.0f%%", startangle=90, colors=colors, textprops={"color": "#c9d1d9", "fontsize": 9})
    ...
    if adds:
        ...
        for cat in sorted(set(cats)):
            ...
            ax2.scatter([d for d, m in zip(diffs, mask) if m], [i for i, m in zip(imps, mask) if m], label=cat, s=60, alpha=0.7, edgecolors="#30363d")
    ax2.set_title("Difficulty vs Importance", color="#f0f6fc", fontweight="bold")
    ax2.set_xlabel("Difficulty"); ax2.set_ylabel("Importance")
    ax2.set_xlim(0, 1); ax2.set_ylim(0, 1)
    ax2.legend(fontsize=8, framealpha=0.3); ax2.grid(True, alpha=0.2)
    ax3 = fig.add_subplot(gs[0, 2])
    durs = [r["duration"] for r in records if r["duration"] is not None and r["duration"] > 0]
    if durs:
    ...
    ax3.set_xlabel("Minutes"); ax3.set_ylabel("Count")
    ...
    ax5.bar(["Add", "Modify"], [len(adds), len(modifies)], color=[cfg["accent"], "#bc8cff"], edgecolor="#30363d")
    ...
    for i, v in enumerate([len(adds), len(modifies)]):
        ax5.text(i, v + mx * 0.05, str(v), color="#c9d1d9", ha="center", fontweight="bold")
    ax6 = fig.add_subplot(gs[1, 2])
    ax6.axis("off")
    deadlines = Counter(r["deadline"] for r in records if r["deadline"])
    dl_text = "\n".join(f"  {k}: {v}" for k, v in deadlines.most_common(8)) if deadlines else "  (none)"
    ax6.text(0.05, 0.5, f"DEADLINES\n{dl_text}", fontsize=11, family="monospace", color="#c9d1d9", va="center")
...
def plot_categories(records, out_dir, cfg):
    ...
    ax.set_title(f"Category Distribution — {cfg['title_suffix']}", color="#f0f6fc", fontweight="bold")
    ax.set_xlabel("Count")
...
def plot_scatter(records, out_dir, cfg):
    adds = [r for r in records if r["type"] == "add" and r["difficulty"] is not None and r["importance"] is not None]
    if not adds:
        return
    ...
    for cat in sorted(set(cats)):
        mask = [c == cat for c in cats]
        ax.scatter([d for d, m in zip(diffs, mask) if m], [i for i, m in zip(imps, mask) if m], label=cat, s=60, alpha=0.7, edgecolors="#30363d")
    ax.set_title(f"Difficulty vs Importance — {cfg['title_suffix']}", color="#f0f6fc", fontweight="bold")
    ax.set_xlabel("Difficulty"); ax.set_ylabel("Importance")
    ax.set_xlim(0, 1); ax.set_ylim(0, 1)
    ax.legend(fontsize=9, framealpha=0.3); ax.grid(True, alpha=0.2)
    plt.tight_layout()
...
def plot_difficulty(records, out_dir, cfg):
    ...
    ax.set_title(f"Difficulty Distribution — {cfg['title_suffix']}", color="#f0f6fc", fontweight="bold")
    ax.set_xlabel("Difficulty"); ax.set_ylabel("Count")
    ax.set_xlim(0, 1); ax.grid(True, alpha=0.2, axis="y")
    plt.tight_layout()
...
def plot_importance(records, out_dir, cfg):
    ...
    ax.set_title(f"Importance Distribution — {cfg['title_suffix']}", color="#f0f6fc", fontweight="bold")
    ax.set_xlabel("Importance"); ax.set_ylabel("Count")
    ax.set_xlim(0, 1); ax.grid(True, alpha=0.2, axis="y")
    plt.tight_layout()
...
def plot_duration(records, out_dir, cfg):
    vals = [r["duration"] for r in records if r["duration"] is not None and r["duration"] > 0]
    if not vals:
        return
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.hist(vals, bins=20, color="#f78166", edgecolor="#30363d", alpha=0.8)
    ax.set_title(f"Duration Distribution (min) — {cfg['title_suffix']}", color="#f0f6fc", fontweight="bold")
...
def plot_features(records, out_dir, cfg):
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.barh(labels, values, color=[cfg["accent"], "#bc8cff", "#f78166", "#58a6ff", "#3fb950"], edgecolor="#30363d")
    ax.set_title(f"Feature Coverage — {cfg['title_suffix']}", color="#f0f6fc", fontweight="bold")
    ax.set_xlabel("Count")
    ax.grid(True, alpha=0.15, axis="x")
    for i, v in enumerate(values):
        ax.text(v + max(values) * 0.01, i, str(v), color="#c9d1d9", va="center")
...
def main():
    ...
    parser.add_argument("--dataset", choices=["real", "specific", "synthetic"], required=True, help="Dataset to plot")
    ...
    if not examples:
        print(f"No examples found in {path} (synthetic data uses templates, not examples)")
        sys.exit(0)
