...
def plot_overview(data, out_dir):
    ...
    fig.suptitle("VM.AI Synthetic Dataset Overview", color="#f0f6fc", fontsize=16, fontweight="bold", y=0.95)

    ax1 = fig.add_subplot(gs[0, 0])
    items = [
        ("templates", len(templates)), ("tasks", len(tasks)),
        ("deadlines", len(deadlines)), ("durations", len(durations)),
        ("dates", len(dates)), ("times", len(times)),
        ("locations", len(locations)), ("categories", len(categories)),
        ("priorities", len(priorities)), ("difficulties", len(difficulties)),
    ...
    ax2.set_title("Placeholder Usage Across Templates", color="#f0f6fc", fontweight="bold")
    ...
    ax3.pie([1] * len(cats), labels=cats, startangle=90, textprops={"color": "#c9d1d9", "fontsize": 9}, colors=plt.cm.tab10(range(len(cats))))
    ...
    ax5.hist(field_counts_per_template, bins=range(1, max(field_counts_per_template) + 2), color="#f78166", edgecolor="#30363d", alpha=0.8)
    ax5.set_title("Fields Per Template", color="#f0f6fc", fontweight="bold")
    ax5.set_xlabel("Number of Placeholders"); ax5.set_ylabel("Count")
    ax5.grid(True, alpha=0.2, axis="y")

    ax6 = fig.add_subplot(gs[1, 2])
    ax6.axis("off")
    ph_text = "\n".join(f"  [{ph}]: {c}" for ph, c in ph_counts.most_common())
    ax6.text(0.05, 0.5, f"PLACEHOLDER COVERAGE\n{ph_text}", fontsize=11, family="monospace", color="#c9d1d9", va="center")
...
def plot_template_complexity(data, out_dir):
    ...
    ax.hist(field_counts, bins=range(1, max(field_counts) + 2), color="#bc8cff", edgecolor="#30363d", alpha=0.8)
    ax.set_title("Template Complexity (Fields Per Template)", color="#f0f6fc", fontweight="bold")
    ax.set_xlabel("Number of Placeholders"); ax.set_ylabel("Template Count")
...
def plot_placeholder_usage(data, out_dir):
    ...
    ax.set_title("Placeholder Frequency Across Templates", color="#f0f6fc", fontweight="bold")
...
def plot_importance_distribution(data, out_dir):
    ...
    ax.set_title("Priority/Importance Value Distribution", color="#f0f6fc", fontweight="bold")
...
def plot_duration_distribution(data, out_dir):
    ...
    ax.set_title("Duration Value Distribution (minutes)", color="#f0f6fc", fontweight="bold")
