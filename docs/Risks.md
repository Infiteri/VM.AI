...
**✅ Proposed Solution:**
1. Hard timeout: `if time.time() - start > 12: break`
2. Early termination: `if best_score < 0.35: leave_unscheduled(task_id)`
3. Limit candidate slots to top 15 per task before full scoring.
4. Log displacement depth for tuning.  
...
**✅ Proposed Solution:** Document the invariant clearly. Add a SQL comment to the schema: `-- When association_status='same', multiple tasks share this row. Manual cleanup only when reference_count=0.` Add `reference_count` post-competition if needed.  
...
**✅ Proposed Solution:**
1. Enable weekly `×0.99` decay as a default background job.
2. Compute radial boosts in Python, then apply in one atomic query:
```python
# Python: build update_dict = {"10:00": 2.5, "10:15": 1.75}
...
**What happens:** Backend and frontend are built without documented endpoints. Payload shapes, error formats, and state transitions are assumed.  
**Impact:** 5–7 days wasted on integration debugging right before demo.  
**✅ Proposed Solution:** Draft a minimal OpenAPI/JSON contract for 5 core endpoints (`/tasks/add`, `/schedule`, `/commit`, `/tasks/{id}/rate`, `/stats`). Frontend mocks responses first. Backend matches shapes exactly.  
⏱️ **Time:** 2 hours | **Result:** Zero integration surprises, parallel dev possible.
...
|------|-------|--------------|
| **Week 1** | Foundation & Safety | ✅ Transactional commit<br>✅ Synchronous Stats Recorder<br>✅ Pydantic NLP validation<br>✅ DB indexes<br>✅ `dateparser` strict settings |
| **Week 2** | Core Pipeline & Math | ✅ Running average formulas (records vs completed_count)<br>✅ Category stats mirroring<br>✅ Scheduler timeout + early termination<br>✅ Radial decay in Python → `jsonb_set` |
| **Week 3** | Demo Readiness | ✅ Pre-seed synthetic stats<br>✅ Draft 5 API contracts<br>✅ Add basic logging/metrics per stage<br>✅ Record backup demo video |
| **Week 4** | Polish & Freeze | ✅ Performance test (50+ tasks)<br>✅ Write 1-page failure runbook<br>✅ Freeze code 48h before demo<br>✅ Only fix critical bugs |
...
**Next Step:** Generate production-ready `stable_scheduler.py` or 5-endpoint JSON contract.
