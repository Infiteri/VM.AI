...
- **Rated flag** — `tasks.rated` (BOOLEAN) tracks if a completed task has been user-rated. Default: `FALSE`. Updated synchronously by Stats Recorder.
...
│ • recurrent (unused)        │                     │ • change_type   │
│ • recurrence_days (unused)  │                     │ • old_slot_*    │
└──────────────┬──────────────┘                    │ • new_slot_*    │
               │                                   │ • created_at    │
                ▼                                   └─────────────────┘
┌─────────────────────────────────────────────────┐
...
| **rated** | **BOOLEAN** | **Tracks if user has rated this task. Default: FALSE. Used by UI to show rating state.** |
