...
The Parser is the first stage of the VM.AI pipeline. It takes natural language input and turns it into structured task data for the rest of the system. It uses a hybrid approach: a rule-based parser handles add mode (T5 has token leakage issues there), while a fine-tuned T5-base model handles modify mode with rule-based fallbacks for importance and difficulty.
...
Add mode is purely rule-based via `rule_based_add.py`. The rule-based parser strips common prefixes like "I need to", "I have to", "schedule", "remind me to". It extracts the task name by cutting off at keywords like " at ", " by ", " for ". Time is extracted by regex and normalized. Duration checks for explicit mentions first, then falls back to a task-type map (gym→45, meeting→30, code→60, study→60, etc.). Recurrence is detected from "every"/"daily"/"each". Difficulty and importance use keyword maps (hard→0.85, urgent→0.95, easy→0.15, optional→0.2) with task-type fallbacks. Deadline is matched from "tomorrow", "next week", or day names. Location uses phrase matching. Category uses a large keyword map mapping tasks to categories. Defaults are difficulty=0.5, importance=0.5, duration=30, category="personal".

### Modify Mode

Modify mode is hybrid. The rule-based `parse_modify_rule_based` handler handles importance and difficulty changes with explicit keyword→value maps (same as add mode). The T5 model handles deadline, time, location, category, and recurrence changes. The chat script merges both: rules always win for importance and difficulty, the model fills everything else. If a field appears in both and the model returned a real change, the rule-based version for non-importance/difficulty fields can also supplement.
